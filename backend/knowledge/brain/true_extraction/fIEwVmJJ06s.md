...
- **Implications pour le bot (majeures)** :
  - **Si on accepte cette vidéo**, notre hypothèse "MASTER mal compris" (operator spec) est **partiellement fausse** : ce n'est pas que MASTER soit mal compris, c'est que **MASTER étant public, son edge est déjà arbitragé**. 10 data points négatifs = cohérent avec "edge mort" plutôt que "implémentation imparfaite".
